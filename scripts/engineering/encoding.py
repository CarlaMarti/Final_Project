...
def encoding_categorical(df_to_engineer):
    ...
    df_to_engineer["!ATTRITION_FLAG!"] = df_to_engineer["Attrition_Flag"].astype('category').cat.codes
    ...
    for category in columns_to_encode:
        ...
        df_encoded = df_encoded.apply(lambda x: x.map({True: 1, False: 0}))  # Convertir True a 1 y False a 0
