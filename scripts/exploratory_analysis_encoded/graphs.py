...
def visualize_variables(df):
    ...
    fig.suptitle(
        "CANTIDAD MONETARIA EN TRANSACCIONES Y NÚMERO DE TRANSACCIONES", fontsize=20
    )
    ...
    sns.histplot(
        data=df, x="Total_Trans_Amt", hue="Attrition_Flag", ax=ax3, multiple="stack"
    )
    sns.histplot(
        data=df, x="Total_Trans_Ct", hue="Attrition_Flag", ax=ax4, multiple="stack"
    )

    heatmap_ = pd.crosstab(df["Attrition_Flag"], df["Total_Trans_Amt"])
    row_perc = heatmap_.div(heatmap_.sum(axis=1), axis=0) * 100
    sns.heatmap(row_perc, annot=True, fmt=".1f", cmap="Blues", linewidths=0, ax=ax5)
    ax5.set_title("Heatmap Total_Trans_Amt")

    heatmap_ = pd.crosstab(df["Attrition_Flag"], df["Total_Trans_Ct"])
    row_perc = heatmap_.div(heatmap_.sum(axis=1), axis=0) * 100
    sns.heatmap(row_perc, annot=True, fmt=".1f", cmap="Blues", linewidths=0, ax=ax6)
    ...
    fig.suptitle("NUMERO DE PRODUCTOS (TOTAL RELATIONSHIP COUNT)", fontsize=20)

    sns.boxplot(data=df, x="Total_Relationship_Count", y="Attrition_Flag", ax=ax1)
    ax1.set_xlabel("Number of products")
    ax1.set_title("Number of bank products contracted")
    ax1.set_ylabel("")

    counts = df.groupby(["Total_Relationship_Count", "Attrition_Flag"]).size().unstack()
    ...
    ax3.bar(
        attrited_customer.index, attrited_customer.values, label="Attrited Customer"
    )
    ...
    ax3.set_title("Number of bank products contracted - Attrited Customer")
    ax3.legend()

    ax4.bar(
        existing_customer.index, existing_customer.values, label="Existing Customer"
    )
    ...
    ax4.set_title("Number of bank products contracted - Existing Customer")
    ax4.legend()
    ...
    # Gráfico 3
    fig, ((ax1, ax2), (ax3, ax4)) = plt.subplots(nrows=2, ncols=2, figsize=(15, 15))
    fig.suptitle("NUMERO DE MESES INACTIVOS EN EL ÚLTIMO AÑO", fontsize=20)

    sns.boxplot(data=df, x="Months_Inactive_12_mon", y="Attrition_Flag", ax=ax1)
    ax1.set_xlabel("Number of months")
    ax1.set_title("Months inactive in the last year")
    ax1.set_ylabel("")

    counts = df.groupby(["Months_Inactive_12_mon", "Attrition_Flag"]).size().unstack()
    counts.plot(kind="bar", stacked=True, ax=ax2)
